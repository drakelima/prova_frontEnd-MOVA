<template>
  ...
  <v-container v-show="!viewPais">
    ...
    <v-flex d-flex>
      <v-layout wrap>
        <v-flex md4 sm4 class="pl-6 pr-6 pa-4" v-for="item in paisesPaginados" :key="item.id">
  ...
  <v-container v-show="viewPais" grid-list-xl class="mt-md-12">
      <v-layout row wrap>
          ...
          <v-flex d-flex xs12 sm6 md7>
              <v-flex d-flex>
                <v-layout row wrap>
                    <v-card elevation="0">
                      <v-card-subtitle> Nome: {{pais.name}} </v-card-subtitle>
                      <v-card-subtitle> Capital: {{pais.capital}} </v-card-subtitle>
                      <v-card-subtitle> Região: <span class="linkClickable" @click="[selectOne = 1, option = 1, regionId = pais.region, onBackChild(false), goFilter()]"> {{pais.region}} </span> </v-card-subtitle>
                      <v-card-subtitle> Sub-região: {{pais.subregion}} </v-card-subtitle>
                      <v-card-subtitle> População: {{pais.population}} </v-card-subtitle>
                      <v-card-subtitle> Línguas: <span v-for="language in pais.languages" :key="language.index">{{language.nativeName}}, </span></v-card-subtitle>
                    </v-card>
                </v-layout>
              </v-flex>
          </v-flex>
      ...
      <v-layout row wrap>
        <v-flex d-flex>
          <v-layout wrap>
            <v-flex md4 sm4 v-for="item in paisesVizinhosPaginados.slice(0,3)" :key="item.id">
              <v-hover v-slot="{ hover }">
                <v-card class="card-container">
                  <v-img 
    ...
    methods: {
      ...
      async getPaises(){
        await this.axios.get('all?fields=name;flag;region;capital;languages;alpha2Code;currencies;population;subregion;borders').then((res) => {
          ...
          this.paisesPaginados = res.data.slice((this.page - 1)* this.perPage, this.page* this.perPage)
        }).finally(() => {
          this.getRegioes()
          this.getCapitais()
          this.getLinguas()
      ...
      visiblePages(){
        this.paisesPaginados = this.paises.slice((this.page - 1)* this.perPage, this.page* this.perPage)
        this.loadingPesquisar = false
      },
      visiblePagesVizinhos(){
        this.paisesVizinhosPaginados = this.paisesVizinhos.slice((this.pageVizinho - 1)* this.perPageVizinho, this.pageVizinho* this.perPageVizinho)
      },
      ...
      async getDataPais(alpha){
        ...
        await this.axios.get(`alpha/${alpha}?fields=name;flag;region;capital;languages;alpha2Code;population;subregion;borders`).then((res) => {
          ...
          this.paisesVizinhosPaginados = this.paisesVizinhos
          this.viewPais = true
        })
      },
    },
